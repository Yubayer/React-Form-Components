/**
 * @fileoverview Production-Grade Form Component System — v3
 *
 * v3 additions over v2:
 *  - CheckboxField        — single checkbox (TextField-style)
 *  - RadioField           — single radio button (TextField-style)
 *  - ColorPickerField     — restyled to match TextField visual language
 *  - EmailField           — validateFormat prop (opt-in email format check)
 *  - All fields           — floatingLabel prop (animated float-up label)
 *  - All fields           — fieldset + legend prop (wraps field in <fieldset>)
 *
 * Components:
 *  - FormFieldMessage     — shared helper/error message renderer
 *  - TextField            — text input with typing indicator
 *  - EmailField           — email input with live validation + optional format check
 ...
 *  - RadioField           — single radio button with label + description
 *  - CheckboxGroup        — group of checkboxes (horizontal/vertical)
 *  - CheckboxField        — single checkbox with label + description
 ...
 *  - ColorPickerField     — color selection, swatches or input variant
 *  - BooleanField         — yes/no pill selector
 *  - JsonField            — JSON textarea with live parse validation
...
function FieldLabel({ htmlFor, label, required, info, className = "", floating = false, floatActive = false }) {
    if (!label) return null;

    const labelClass = `ff-label ${floating ? "ff-label--floating" : ""} ${floating && floatActive ? "ff-label--float-active" : ""} ${className}`;

    if (info) {
        return (
            <div className={`ff-label-row ${floating ? "ff-label-row--floating" : ""}`}>
...
/* ═══════════════════════════════════════════════════════════════════════════════
   SHARED: FieldsetField — exported group wrapper
   Groups multiple fields inside a real <fieldset> + <legend>.
   Use this when you want to visually group several unrelated fields.
   For a single field with a legend on its own border, pass fieldset+legend
   props directly on the field component — no outer wrapper is added.
   ...
   <FieldsetField legend="Shipping Address">
     <TextField label="Street" value={street} setValue={setStreet} />
     <TextField label="City"   value={city}   setValue={setCity} />
   </FieldsetField>
═══════════════════════════════════════════════════════════════════════════════ */

export function FieldsetField({ legend = "", children, className = "" }) {
    return (
        <fieldset className={`ff-fieldset-group ${className}`}>
            {legend && <legend className="ff-fieldset-group-legend">{legend}</legend>}
            {children}
        </fieldset>
    );
}
...
/* ═══════════════════════════════════════════════════════════════════════════════
   INTERNAL: InlineLegend
   Sits inside ff-input-wrap so it renders on the input's own border.
   Only mounted when fieldset=true + legend is set on a field.
═══════════════════════════════════════════════════════════════════════════════ */

function InlineLegend({ legend, required }) {
    if (!legend) return null;
    return (
        <span className="ff-inline-legend" aria-hidden="true">
            {legend}{required && <span className="ff-required"> *</span>}
        </span>
    );
}
...
/* ═══════════════════════════════════════════════════════════════════════════════
   SHARED: useThrottle
   — Returns a throttled version of any callback.
   — delay=0 means no throttling (passthrough).
═══════════════════════════════════════════════════════════════════════════════ */
...
function useThrottle(fn, delay) {
    ...
    return useCallback((...args) => {
        if (!delay) { fn?.(...args); return; }
        const now = Date.now();
        const remaining = delay - (now - lastFired.current);
...
/* ═══════════════════════════════════════════════════════════════════════════════
   SHARED: useDebounce
   — Returns a debounced version of any callback.
   — delay=0 means no debouncing (passthrough).
═══════════════════════════════════════════════════════════════════════════════ */
 ...
 * />
 *
 * // Throttle onChange to fire at most once per 300ms:
 * // <TextField label="Search" value={q} setValue={setQ} throttle={300} onChange={handleSearch} />
 *
 * // Floating label:
 * // <TextField label="Full Name" value={name} setValue={setName} floatingLabel />
 *
 * // Legend on input border (label auto-hidden):
 * // <TextField label="Full Name" value={name} setValue={setName} fieldset legend="Personal Info" />
 *
 * // Hide label:
 * // <TextField label="Full Name" value={name} setValue={setName} hideLabel />
 */
export function TextField({
    ...
    const floatActive = floatingLabel && (isFocused || Boolean(value));

    const throttledOnChange = useThrottle(onChange, throttleMs);

    const handleChange = useCallback((e) => {
        const v = e.target.value;
        setValue?.(v);
        throttledOnChange(v, e);
        triggerTyping();
    }, [setValue, throttledOnChange, triggerTyping]);

    const showLabel = !hideLabel && !fieldset && !floatingLabel;

    return (
        <div className={`ff ${floatingLabel ? "ff--floating" : ""} ${className}`}>
            {showLabel && <FieldLabel htmlFor={inputId} label={label} required={required} info={info} />}
            <div className={`ff-input-wrap ${fieldset ? "ff-input-wrap--fieldset" : ""}`}>
                {fieldset && <InlineLegend legend={legend || label} required={required} />}
                ...
                />
                {floatingLabel && (
                    <FieldLabel htmlFor={inputId} label={label} required={required} info={info} floating floatActive={floatActive} />
                )}
 ...
 * />
 *
 * // Disable format check (server-side validation only):
 * // <EmailField label="Email" value={email} setValue={setEmail} validateFormat={false} />
 *
 * // Floating label:
 * // <EmailField label="Email Address" value={email} setValue={setEmail} floatingLabel />
 */
 ...
 * />
 *
 * // Throttle onChange — useful for live preview / autosave:
 * // <TextArea label="Notes" value={notes} setValue={setNotes} throttle={500} onChange={autoSave} />
 */
...
export function RadioGroup({
    ...
    const showGroupLabel = !hideLabel && !fieldset;

    return (
        <div
            className={`ff ${fieldset ? "ff-group--fieldset" : ""} ${className}`}
            role="radiogroup"
            aria-required={required}
            aria-invalid={hasError}
        >
            {fieldset && <span className="ff-group-inline-legend">{legend || label}{required && <span className="ff-required"> *</span>}</span>}
            {showGroupLabel && label && <div className="ff-label">{label}{required && <span className="ff-required">*</span>}</div>}
...
/* ═══════════════════════════════════════════════════════════════════════════════
   12b. RadioField — single radio button (TextField-style)
═══════════════════════════════════════════════════════════════════════════════ */

/**
 * RadioField — A single styled radio button with label and optional description.
 * Styled to match TextField visual language (border, glow, surface).
 * Useful for yes/no choices, opt-in rows, or standalone option cards.
 *
 * @param {Object}   props
 * @param {string}   [props.label]                  - Main radio text
 * @param {string}   [props.description]            - Sub-text below label
 * @param {boolean}  [props.checked=false]          - Controlled selected state
 * @param {Function} [props.onChange]               - (checked: boolean) => void
 * @param {Function} [props.setValue]
 * @param {boolean}  [props.showSelectedStyle=false] - When true, applies gold border + glow when selected.
 *                                                     Default false — selected state only shows dot, no visual row change.
 * @param {boolean}  [props.disabled=false]
 * @param {boolean}  [props.required=false]
 * @param {string}   [props.helperText]
 * @param {string}   [props.error]
 ...
 * />
 *
 * // Gold row highlight when selected:
 * // <RadioField label="Pay on delivery" checked={method === 'cod'} onChange={() => setMethod('cod')} showSelectedStyle />
 *
 * // With fieldset:
 * // <RadioField label="Pay on delivery" checked={method === 'cod'} onChange={() => setMethod('cod')} fieldset legend="Payment Method" />
 */
export function RadioField({
    ...
    const toggle = useCallback(() => {
        ...
        setValue?.(!checked);
        onChange?.(!checked);
    }, [checked, disabled, setValue, onChange]);
    ...
    return (
        <div className={`ff ${className}`}>
            <div
                ...
                className={`ff-radio-single ${fieldset ? "ff-radio-single--fieldset" : ""}
                        ${showSelectedStyle && checked ? "ff-radio-single--selected" : ""}
                        ${hasError ? "ff-radio-single--error" : ""}
                        ${disabled ? "ff-radio-single--disabled" : ""}
                    `}
                onClick={toggle}
                onKeyDown={(e) => { if (e.key === " " || e.key === "Enter") { e.preventDefault(); toggle(); } }}
            >
                <span className={`ff-radio-circle ${checked ? "ff-radio-circle--dotted" : ""}`}>
                    <span className="ff-radio-dot" />
                </span>
                <span className="ff-radio-single-text">
                    {label && (
                        <span className="ff-radio-single-label">
                            {label}
                            {required && <span className="ff-required" aria-hidden="true">*</span>}
                        </span>
                    )}
...
export function CheckboxGroup({
    ...
    return (
        <div className={`ff ${fieldset ? "ff-group--fieldset" : ""} ${className}`} role="group" aria-invalid={hasError}>
            {fieldset && <span className="ff-group-inline-legend">{legend || label}{required && <span className="ff-required"> *</span>}</span>}
            {showGroupLabel && label && <div className="ff-label">{label}{required && <span className="ff-required">*</span>}</div>}
...
/* ═══════════════════════════════════════════════════════════════════════════════
   13b. CheckboxField — single checkbox (TextField-style)
═══════════════════════════════════════════════════════════════════════════════ */

/**
 * CheckboxField — A single styled checkbox with label and optional description.
 * Styled to match TextField visual language (border, glow, surface).
 *
 * @param {Object}   props
 * @param {string}   [props.label]                  - Main checkbox text
 * @param {string}   [props.description]            - Sub-text below label
 * @param {boolean}  [props.value=false]            - Controlled checked state
 * @param {Function} [props.onChange]               - (checked: boolean) => void
 * @param {Function} [props.setValue]
 * @param {boolean}  [props.showCheckedStyle=false] - When true, applies gold border + glow when checked.
 *                                                    Default false — checked state only shows tick, no visual row change.
 * @param {boolean}  [props.disabled=false]
 * @param {boolean}  [props.required=false]
 * @param {string}   [props.helperText]
 * @param {string}   [props.error]
 ...
 * />
 *
 * // Gold row highlight when checked:
 * // <CheckboxField label="Subscribe to newsletter" value={sub} setValue={setSub} showCheckedStyle />
 *
 * // With fieldset:
 * // <CheckboxField label="Subscribe to newsletter" value={sub} setValue={setSub} fieldset legend="Preferences" />
 */
export function CheckboxField({
    ...
    const uid = useId();
    const checkId = id || `ff-checkbox-${uid}`;
    const hasError = showError && (Boolean(error) || Boolean(errors?.length));
    ...
    return (
        <div className={`ff ${className}`}>
            <div
                ...
                aria-required={required}
                tabIndex={disabled ? -1 : 0}
                className={`ff-checkbox-single ${fieldset ? "ff-checkbox-single--fieldset" : ""}
                        ${showCheckedStyle && value ? "ff-checkbox-single--checked" : ""}
                        ${hasError ? "ff-checkbox-single--error" : ""}
                        ${disabled ? "ff-checkbox-single--disabled" : ""}
                    `}
                onClick={toggle}
                onKeyDown={(e) => { if (e.key === " " || e.key === "Enter") { e.preventDefault(); toggle(); } }}
            >
                <span className={`ff-checkbox-box ${value ? "ff-checkbox-box--ticked" : ""}`}>
                    <span className="ff-checkbox-tick"><Icon.Check /></span>
                </span>
                <span className="ff-checkbox-single-text">
                    {label && (
                        <span className="ff-checkbox-single-label">
                            {label}
                            {required && <span className="ff-required" aria-hidden="true">*</span>}
                        </span>
                    )}
 ...
 * @param {boolean}            [props.value=false]              - Controlled on/off state
 * @param {Function}           [props.onChange]                 - (value: boolean) => void
 * @param {Function}           [props.setValue]
 * @param {"left"|"right"}     [props.switchPosition="right"]   - Toggle switch position relative to label text
 * @param {boolean}            [props.showOnClass=false]        - When true, adds `ff-switch-row--on` class regardless of value; decouples CSS row state from toggle state
 * @param {boolean}            [props.disabled=false]
 * @param {string}             [props.helperText]
 * @param {string}             [props.error]
 * @param {boolean}            [props.showError=true]
 * @param {Array}              [props.errors=[]]
 * @param {string}             [props.info]                     - Tooltip text shown next to the label
 * @param {string}             [props.className]
 */
export function SwitchField({
    ...
    const rowClass = `ff-switch-row ${showOnClass ? "ff-switch-row--on" : ""} ${disabled ? "ff-switch-row--disabled" : ""} ${switchPosition === "left" ? "ff-switch-row--left" : ""}`;
 ...
 * @param {string|string[]}          [props.previewUrl]               - Existing URL(s) from database to preview. Single string for single mode; string array for multi mode.
 * @param {Function}                 [props.onPreviewRemove]          - (url: string, index: number) => void. Called when user removes an existing URL preview. Works in both single and multi mode. If omitted, no remove button is shown on existing previews.
 *
 * @example
 * // Default card style:
...
export function ImagePickerField({
    ...
    if (!multiple && variant === "avatar") {
        const hasImage = previewMap.length === 1 || Boolean(previewUrl);
        const file = previewMap.length === 1 ? previewMap[0].file : null;
        ...
        const imageSrc = previewMap?.[0]?.url || previewUrl || null;
        ...
        return (
            <div className={`ff ff--picker ${className}`}>
                ...
                <div className={`ff-avatar-wrap ${positionClass}`}>
                    ...
                    >
                        {imageSrc ? (
                            <img src={imageSrc} alt={file?.name || "Preview"} className="ff-avatar-img" />
                        ) : (
                                ...
                                <span className="ff-avatar-overlay-actions">
                                    ...
                                    <span
                                        ...
                                        onClick={(e) => {
                                            ...
                                            if (file) {
                                                remove(0);
                                            } else if (onPreviewRemove) {
                                                onPreviewRemove(previewUrl, 0);
                                            }
                                        }}
                                    >
                                        <Icon.X />
    ...
    if (!multiple && (previewMap.length === 1 || (!multiple && previewMap.length === 0 && previewUrl))) {
        const file = previewMap[0]?.file || null;
        const thumbSrc = previewMap[0]?.url || previewUrl;
        return (
            <div className={`ff ff--picker ${className}`}>
                {label && (
                    <div className="ff-label-row">
                    ...
                    <div className="ff-sp-footer">
                        <div className="ff-sp-meta">
                            ...
                            <div className="ff-sp-meta-text">
                                <span className="ff-sp-name">{file?.name || previewUrl}</span>
                                {file?.size && <span className="ff-sp-size">{formatFileSize(file.size)}</span>}
                            </div>
                        </div>
                        <div className="ff-sp-actions">
                            ...
                            {file ? (
                                ...
                                <button
                                    type="button"
                                    className="ff-sp-remove"
                                    onClick={() => onPreviewRemove(previewUrl, 0)}
                                    disabled={disabled}
                                    aria-label="Remove image"
                                >
                                    <Icon.X />
                ...
                return previewMode === "list" ? (
                    <div className="ff-media-list ff-media-list--image">
                        {existingUrls.map((url, i) => {
                            ...
                            return (
                                <div key={`existing-img-${i}`} className="ff-media-list-item">
                                    <div className="ff-media-list-thumb ff-media-list-thumb--image">
                                        <img src={url} alt={`Existing ${i + 1}`} className="ff-media-list-thumb-img" />
                                    </div>
                                    <div className="ff-media-list-info">
                                        <span className="ff-media-list-name">{baseName}</span>
                                        {ext && <div className="ff-media-list-meta"><span className="ff-media-list-ext">{ext}</span></div>}
                                    </div>
                                    {onPreviewRemove && (
                                        <button type="button" className="ff-media-list-remove" onClick={() => onPreviewRemove(url, i)} aria-label="Remove image">
                                            <Icon.X />
                                        </button>
                                    )}
                                </div>
                    ...
                    <div className="ff-preview-grid">
                        {existingUrls.map((url, i) => (
                            <div key={`existing-img-${i}`} className="ff-preview-item">
                                <img src={url} alt={`Existing ${i + 1}`} className="ff-preview-media" />
                                {onPreviewRemove && (
                                    <div className="ff-preview-overlay">
                                        <button type="button" className="ff-preview-remove" onClick={() => onPreviewRemove(url, i)} aria-label="Remove image">
                                            <Icon.X />
                                        </button>
                                    </div>
                                )}
 ...
 * Multi mode: shows a list or grid of video thumbnails below the upload zone.
 *
 * @param {Object}             props
 * @param {string}             [props.label]
 * @param {File[]}             [props.value=[]]
 ...
 * @param {number}             [props.maxSize]               - Bytes e.g. 100 * 1024 * 1024
 * @param {"list"|"grid"}      [props.previewMode="list"]    - Multi preview style
 * @param {string}             [props.previewWidth]
 * @param {"card"|"avatar"}    [props.variant="card"]        - Single-file display style. "avatar" shows a circular video thumbnail.
 * @param {string|string[]}    [props.previewUrl]            - Existing URL(s) from database to preview. Single string for single mode; string array for multi mode.
 * @param {Function}           [props.onPreviewRemove]       - (url: string, index: number) => void. Called when user removes an existing URL preview. Works in both single and multi mode. If omitted, no remove button is shown on existing previews.
 * @param {string}             [props.helperText]
 * @param {string}             [props.error]
 ...
 * />
 *
 * // Single with existing DB url:
 * <VideoPickerField
 *   label="Intro Video"
 *   value={newVideo}
 *   setValue={setNewVideo}
 *   multiple={false}
 *   previewUrl="https://cdn.example.com/intro.mp4"
 * />
 *
 * // Multi with existing DB urls + remove callback:
 * <VideoPickerField
 *   label="Course Videos"
 *   value={newVideos}
 ...
 *   multiple
 *   maxFiles={5}
 *   previewUrl={existingVideoUrls}
 *   onPreviewRemove={(url, i) => setExistingVideoUrls(prev => prev.filter((_, idx) => idx !== i))}
 * />
 */
export function VideoPickerField({
    ...
    if (!multiple && variant === "avatar") {
        const hasVideo = previewMap.length === 1 || Boolean(previewUrl);
        const file = previewMap.length === 1 ? previewMap[0].file : null;
        const videoSrc = previewMap[0]?.url || previewUrl;
        return (
            <div className={`ff ff--picker ${className}`}>
                {label && (
                    <div className="ff-label-row">
                ...
                <div className="ff-avatar-wrap">
                    ...
                    >
                        {hasVideo ? (
                            <>
                                <video src={videoSrc} className="ff-avatar-img" preload="metadata" muted playsInline />
                    ...
                    {hasVideo && (
                        <div className="ff-avatar-actions">
                            ...
                            <span className="ff-avatar-filename">{file?.name}</span>
                            {file?.size && <span className="ff-avatar-filesize">{formatFileSize(file.size)}</span>}
                        </div>
                    )}
                </div>
                {hasPickerErrors && <FilePickerErrors fileErrors={fileErrors} atomicError={atomicError} externalError={error} showError={showError} />}
                ...
                <div className={`ff-sp-card ${showErr ? "ff-sp-card--error" : ""}`}>
                    ...
                    <div className="ff-sp-footer">
                        <div className="ff-sp-meta">
                            ...
                            <div className="ff-sp-meta-text">
                                <span className="ff-sp-name">{file?.name || videoSrc}</span>
                                {file?.size && <span className="ff-sp-size">{formatFileSize(file.size)}</span>}
                            </div>
                        </div>
                        <div className="ff-sp-actions">
                ...
                return previewMode === "list" ? (
                    <div className="ff-media-list ff-media-list--video">
                        {existingUrls.map((url, i) => {
                            ...
                            const ext = dotIdx > 0 ? fileName.slice(dotIdx + 1).toUpperCase() : "";
                            return (
                                <div key={`existing-vid-${i}`} className="ff-media-list-item">
                                    <div className="ff-media-list-thumb ff-media-list-thumb--video">
                                        <video src={url} className="ff-media-list-thumb-img" muted preload="metadata" />
                                        <span className="ff-media-list-play-icon" aria-hidden="true"><Icon.PlayCircle /></span>
                                    </div>
                                    <div className="ff-media-list-info">
                                        <span className="ff-media-list-name">{baseName}</span>
                                        {ext && <div className="ff-media-list-meta"><span className="ff-media-list-ext ff-media-list-ext--video">{ext}</span></div>}
                                    </div>
                                    {onPreviewRemove && (
                                        <button type="button" className="ff-media-list-remove" onClick={() => onPreviewRemove(url, i)} aria-label="Remove video">
                                            <Icon.X />
                                        </button>
                                    )}
                                </div>
                    ...
                    <div className="ff-preview-grid">
                        {existingUrls.map((url, i) => (
                            <div key={`existing-vid-${i}`} className="ff-preview-item ff-preview-item--video">
                                <video src={url} className="ff-preview-media" muted preload="metadata" />
                                <div className="ff-preview-overlay ff-preview-overlay--always">
                                    ...
                                    {onPreviewRemove && (
                                        <button type="button" className="ff-preview-remove" onClick={() => onPreviewRemove(url, i)} aria-label="Remove video">
                                            <Icon.X />
                                        </button>
                                    )}
                                </div>
 ...
 * @param {Function}         [props.onChange]              - (files: File[]) => void
 * @param {Function}         [props.setValue]
 * @param {string}           [props.accept="*"]            - MIME types or extensions e.g. ".pdf,.docx"
 * @param {boolean}          [props.multiple=true]
 * @param {number}           [props.maxFiles=10]
 * @param {number}           [props.maxSize]               - Bytes
 * @param {"list"|"grid"}    [props.previewMode="list"]    - Multi preview style
 * @param {"card"|"avatar"}  [props.variant="card"]        - Single-file display style
 * @param {string|string[]}  [props.previewUrl]            - Existing URL(s) from database to preview. Single string for single mode; string array for multi mode.
 * @param {string|string[]}  [props.previewFileName]       - Display name(s) for existing URL(s). Used to derive extension and label when the URL alone isn't descriptive. Can be a single string (single mode) or a string array matching previewUrl.
 * @param {Function}         [props.onPreviewRemove]       - (url: string, index: number) => void. Called when user removes an existing URL preview. Works in both single and multi mode. If omitted, no remove button is shown on existing previews.
 * @param {string}           [props.helperText]
 * @param {string}           [props.error]
 * @param {boolean}          [props.showError=true]
 ...
 *   label="Resume / CV"
 *   value={newResume}
 *   setValue={setNewResume}
 *   multiple={false}
 *   previewUrl="https://cdn.example.com/resume.pdf"
 *   previewFileName="resume.pdf"
 * />
 *
 * // Multi with existing DB urls + remove callback:
 * <FilePickerField
 *   label="Attachments"
 *   value={newFiles}
 *   setValue={setNewFiles}
 *   multiple
 *   previewUrl={existingUrls}
 *   previewFileName={existingFileNames}
 *   onPreviewRemove={(url, i) => setExistingUrls(prev => prev.filter((_, idx) => idx !== i))}
 * />
 */
export function FilePickerField({
    ...
    if (!multiple && (internalFiles.length === 1 || (internalFiles.length === 0 && previewUrl))) {
        // getFileType returns { color, bg, label } based on extension
        const ft = getFileType(internalFiles[0]?.name || previewFileName || previewUrl || "");
        return (
            <div className={`ff ff--picker ${className}`}>
                {label && (
                    <div className="ff-label-row">
                ...
                <div className={`ff-sp-card ${showErr ? "ff-sp-card--error" : ""}`}>
                    ...
                    <div className="ff-sp-footer">
                        <div className="ff-sp-meta">
                            ...
                            <div className="ff-sp-meta-text">
                                <span className="ff-sp-name">{internalFiles[0]?.name || previewFileName || previewUrl}</span>
                                {internalFiles[0]?.size && <span className="ff-sp-size">{formatFileSize(internalFiles[0].size)}</span>}
                            </div>
                        </div>
                        <div className="ff-sp-actions">
                            <button
    ...
    return (
        <div className={`ff ff--picker ${className}`}>
            ...
            {/* Existing uploaded URLs (from database) */}
            {(() => {
                const existingUrls = multiple
                    ? (Array.isArray(previewUrl) ? previewUrl : (previewUrl ? [previewUrl] : []))
                    : [];
                ...
                return previewMode === "list" ? (
                    <div className="ff-media-list ff-media-list--file">
                        {existingUrls.map((url, i) => {
                            const rawName = previewFileName
                                ? (Array.isArray(previewFileName) ? (previewFileName[i] || url.split("/").pop()) : previewFileName)
                                : url.split("/").pop() || url;
                            const ft = getFileType(rawName);
                            const dotIdx = rawName.lastIndexOf(".");
                            const baseName = dotIdx > 0 ? rawName.slice(0, dotIdx) : rawName;
                            const ext = dotIdx > 0 ? rawName.slice(dotIdx + 1).toUpperCase() : "";
                            return (
                                <div key={`existing-file-${i}`} className="ff-media-list-item">
                                    ...
                                    <div className="ff-media-list-info">
                                        <span className="ff-media-list-name">{baseName}</span>
                                        {ext && <div className="ff-media-list-meta"><span className="ff-media-list-ext" style={{ color: ft.color, borderColor: `${ft.color}44`, background: ft.bg }}>{ext}</span></div>}
                                    </div>
                                    {onPreviewRemove && (
                                        <button type="button" className="ff-media-list-remove" onClick={() => onPreviewRemove(url, i)} aria-label={`Remove ${rawName}`}>
                                            <Icon.X />
                                        </button>
                                    )}
                                </div>
                    ...
                    <div className="ff-preview-grid ff-preview-grid--file">
                        {existingUrls.map((url, i) => {
                            ...
                            const baseName = dotIdx > 0 ? rawName.slice(0, dotIdx) : rawName;
                            return (
                                <div key={`existing-file-${i}`} className="ff-preview-item ff-preview-item--file">
                                    ...
                                    {onPreviewRemove && (
                                        <button type="button" className="ff-preview-remove ff-preview-remove--file" onClick={() => onPreviewRemove(url, i)} aria-label={`Remove ${rawName}`}>
                                            <Icon.X />
                                        </button>
                                    )}
                                </div>
...
/* ═══════════════════════════════════════════════════════════════════════════════
   20. ColorPickerField
   Three variants: "swatches" (preset palette + optional hex input)
                   "input"    (native color wheel + hex + RGB breakdown)
                   "picker"   (pure native color wheel only, no presets, no hex text)
═══════════════════════════════════════════════════════════════════════════════ */

/**
 * ColorPickerField — Color selection field with three visual styles.
 * Restyled in v3 to match TextField visual language (ff-input wrap, border, glow).
 *
 * @param {Object}    props
 * @param {string}    [props.label]
 ...
 *
 * // Hex input + color wheel trigger:
 * <ColorPickerField label="Accent" value={color} setValue={setColor} variant="input" />
 *
 * // Pure color wheel — debounce onChange by 150ms (good for canvas/preview updates):
 * // <ColorPickerField label="Pick a color" value={color} setValue={setColor} variant="picker" debounce={150} />
 *
 * // With fieldset:
 * // <ColorPickerField label="Brand Color" value={color} setValue={setColor} fieldset legend="Appearance" />
 */
export function ColorPickerField({
    ...
    if (variant === "swatches") {
        return (
            <div className={`ff ${className}`}>
                {!hideLabel && !fieldset && <FieldLabel htmlFor={inputId} label={label} required={required} info={info} />}
                <div className={`ff-color-field-wrap ${fieldset ? "ff-color-field-wrap--fieldset" : ""} ${hasError ? "ff-color-field-wrap--error" : ""} ${isFocused ? "ff-color-field-wrap--focus" : ""}`}>
                    {fieldset && <InlineLegend legend={legend || label} required={required} />}
                    <div className="ff-color-swatches-inline" role="group" aria-label="Color swatches">
                        {swatches.map((hex) => (
                            <button
                                key={hex}
                                type="button"
                    ...
                    {allowCustom && (
                        <div className="ff-color-hex-row">
                            <button
                                type="button"
                            ...
                            <input
                                id={inputId}
                                type="text"
                                className={`ff-color-hex-input-inline ${hasError ? "ff-input--error" : ""}`}
                                value={hexInput}
                                onChange={handleHexChange}
                                onBlur={() => { setIsFocused(false); commit(hexInput); }}
    ...
    /* ── Picker variant — pure native color wheel, no text input, no swatches ── */
    if (variant === "picker") {
        const isHexInvalid = hexInput.length > 0 && !/^#[0-9A-Fa-f]{6}$/.test(hexInput);
        const pickerHasError = hasError || isHexInvalid;

        return (
            <div className={`ff ${className}`}>
                {!hideLabel && !fieldset && <FieldLabel htmlFor={inputId} label={label} required={required} info={info} />}
                <div className={`ff-color-picker-row ${fieldset ? "ff-color-picker-row--fieldset" : ""} ${pickerHasError ? "ff-color-picker-row--error" : ""} ${disabled ? "ff-color-picker-row--disabled" : ""}`}>
                    {fieldset && <InlineLegend legend={legend || label} required={required} />}
                    <input
                        ref={nativeRef}
                        type="color"
                        className="ff-color-picker-swatch"
                        value={/^#[0-9A-Fa-f]{6}$/.test(hexInput) ? hexInput : (value || "#d4a843")}
                        onChange={handleNativeChange}
                        disabled={disabled}
                        aria-hidden="true"
                        tabIndex={-1}
                    ...
                    <input
                        id={inputId}
                        type="text"
                        className={`ff-color-picker-hex-edit ${isHexInvalid ? "ff-color-picker-hex-edit--error" : ""}`}
                        value={hexInput}
                        onChange={handleHexChange}
                        onBlur={() => {
                    ...
                    {/^#[0-9A-Fa-f]{6}$/.test(hexInput) && (
                        <span className="ff-color-picker-rgb" aria-hidden="true">
                            {["R", "G", "B"].map((ch, i) => (
                                <span key={ch} className="ff-color-rgb-chip">
                                    <span className="ff-color-rgb-label">{ch}</span>
                                    <span className="ff-color-rgb-val">{parseInt(hexInput.slice(1 + i * 2, 3 + i * 2), 16)}</span>
                                </span>
                            ))}
                        </span>
                    )}
                    {isHexInvalid && (
                        <span className="ff-color-picker-invalid-icon" aria-hidden="true">
                            <Icon.AlertCircle />
                        </span>
                    )}
                </div>
                {isHexInvalid && (
                    <div className="ff-message ff-message--error" role="alert" aria-live="polite">
                        <span className="ff-message-icon"><Icon.AlertCircle /></span>
                        Invalid hex — must be #rrggbb (e.g. #d4a843)
                    </div>
                )}
                {!isHexInvalid && <FormFieldMessage hasError={hasError} helperText={helperText} error={error} errors={errors} />}
            </div>
        );
    }
    ...
    return (
        <div className={`ff ${className}`}>
            ...
            <div className={`ff-input-wrap ${fieldset ? "ff-input-wrap--fieldset" : ""}`}>
                ...
                <button
                    type="button"
                    className="ff-icon ff-icon--left ff-color-trigger-btn"
                    style={{ background: value || hexInput }}
                    onClick={() => nativeRef.current?.click()}
                    disabled={disabled}
                    aria-label="Open color picker"
                    tabIndex={-1}
                >
                    <span className="ff-color-trigger-icon" aria-hidden="true">
                        <Icon.Palette />
                    </span>
                </button>
                <input
                ...
                <input
                    id={inputId}
                    type="text"
                    className={`ff-input ff-input--has-left ff-input--has-right ff-color-hex-input-main ${fieldset ? "ff-input--no-border" : ""} ${hasError ? "ff-input--error" : ""}`}
                    value={hexInput}
                    onChange={handleHexChange}
                    onBlur={() => { setIsFocused(false); commit(hexInput); }}
                    onFocus={() => setIsFocused(true)}
                ...
                {/^#[0-9A-Fa-f]{6}$/.test(value) && (
                    <span className="ff-icon ff-icon--right ff-color-rgb-badge" aria-hidden="true">
                        {["R", "G", "B"].map((ch, i) => {
 ...
 * />
 *
 * // Debounce — fires onSearch 400ms after user stops typing:
 * // <SearchField label="Search" value={q} setValue={setQ} onSearch={fetchResults} debounce={400} />
 *
 * // Throttle — fires onSearch at most once per 500ms while typing:
 * // <SearchField label="Search" value={q} setValue={setQ} onSearch={fetchResults} throttle={500} />
 */
export function SearchField({
    ...
    const throttleLastFired = useRef(0);
    ...
    // Debounced/throttled onSearch caller — used while typing
    const fireSearch = useCallback((query) => {
        if (!query.trim() || !onSearch) return;

        if (debounceMs > 0) {
            clearTimeout(debounceTimer.current);
            debounceTimer.current = setTimeout(() => onSearch(query.trim()), debounceMs);
            return;
        }

        if (throttleMs > 0) {
            const now = Date.now();
            const remaining = throttleMs - (now - throttleLastFired.current);
            clearTimeout(throttleTimer.current);
            if (remaining <= 0) {
                throttleLastFired.current = now;
                onSearch(query.trim());
            } else {
                throttleTimer.current = setTimeout(() => {
                    throttleLastFired.current = Date.now();
                    onSearch(query.trim());
                }, remaining);
            }
            return;
        }
    }, [onSearch, debounceMs, throttleMs]);
    ...
    }, [setValue, onChange, triggerTyping, fireSearch]);
