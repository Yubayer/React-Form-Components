...
/* ═══════════════════════════════════════════════════════════════════════════════
   14. SwitchField
   NEW: switchPosition prop — "left" puts the toggle before the label text
═══════════════════════════════════════════════════════════════════════════════ */

/**
 * SwitchField — Toggle switch with optional label and description.
 *
 * @param {Object}   props
 * @param {string}   [props.label]           - Section label above the switch row
 * @param {string}   [props.switchLabel]     - Main text in the switch row
 * @param {string}   [props.description]     - Sub-text in the switch row
 * @param {boolean}  [props.value]           - Controlled on/off state
    ...
    return (
        <div className={`ff ${className}`}>
            ...
            <div
                ...
                className={`ff-switch-row
                    ${value ? "ff-switch-row--on" : ""}
                    ${switchPosition === "left" ? "ff-switch-row--left" : ""}
                `}
                onClick={toggle}
                onKeyDown={(e) => { if (e.key === " " || e.key === "Enter") { e.preventDefault(); toggle(); } }}
            >
                {/* Render order changes based on switchPosition prop */}
                {switchPosition === "left" ? (
                    <>
                        {switchTrack}
                        {switchText}
...
export function ImagePickerField({
     ...
     * "avatar" — circular avatar with hover overlay + green status dot.
     */
    variant = "card",
}) {
    const hasError = showError && (Boolean(error) || Boolean(errors?.length));
    ...
    // Activated by variant="avatar". The original "card" variant is untouched below.
        ...
        return (
            <div className={`ff ff--picker ${className}`}>
                ...
                <div className="ff-avatar-wrap">
                    ...
                    <button
                        type="button"
                        className={`ff-avatar-circle ${showErr ? "ff-avatar-circle--error" : ""} ${disabled ? "ff-avatar-circle--disabled" : ""}`}
                    ...
                    >
                        ...
                        {/* Hover overlay — camera + CHANGE */}
                        <span className="ff-avatar-overlay" aria-hidden="true">
                            <Icon.Camera />
                            <span className="ff-avatar-overlay-label">CHANGE</span>
                        </span>
                    </button>
                    {/* Green status dot — only when image selected */}
                    {hasImage && <span className="ff-avatar-dot" aria-hidden="true" />}
                    {/* Remove + meta row */}
                    {hasImage && (
                        <div className="ff-avatar-actions">
                            <button type="button" className="ff-avatar-remove" onClick={() => remove(0)} disabled={disabled} aria-label="Remove image">
